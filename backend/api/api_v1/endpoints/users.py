...
from core.email import send_new_account_email
...
def create_user(
    *,
    ...
    """
    Create new user. This endpoint is only available to users with admin privileges.
    """
...
def read_user_by_id(
    ...
    if not crud.user.is_superuser(current_user):
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, detail="The user doesn't have enough privileges"
        )
    return user


@router.put("/{user_id}", response_model=schemas.User, dependencies=[Depends(deps.get_current_active_superuser)])
def update_user(
    *,
    db: Session = Depends(deps.get_db),
    user_id: int,
